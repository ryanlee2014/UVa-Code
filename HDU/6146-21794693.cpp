...
ll bor[10100]={0,1,3,5};
ll mdidx[10100]={1,2,4,8};
it bValid=3, mValid=3;
using namespace std;
il ll work(it i)
{
    wl (bValid < i)
        bor[++bValid] = (bor[bValid - 1] + bor[bValid - 2] + 1) % mod;
    rtn bor[i];
}
...
{
    wl (mValid < i)
        mdidx[++mValid] = (mdidx[mValid - 1] << 1) % mod;
    rtn mdidx[i];
}
...
it main(vt)
{
    ...
    wl(T--)
    {
        ...
        {
            if (i == 1 || i == n)
                GmidV = (GmidV + work(n)) % mod;
            el
                GmidV = (GmidV + work(i - 1) + work(n - i) % mod) % mod;
        }
        fin << (GmidV * idx(n)) % mod << ln;
    }
    rtn 0;
}