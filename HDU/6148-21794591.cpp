...
it main() {
    ...
    wl (T--) {
        ...
        rep(i,1,len)
        {
            ...
            rep(j,0,10)
            {
                rep(k,0,j)dp2[i][j] += dp2[i - 1][k] + dp1[i - 1][k];
                dp2[i][j] += dp2[i - 1][j];
            }
            rep(j,0,10)
            {
                rvep(k,9,j+1)dp1[i][j] += dp1[i - 1][k],sum += dp2[i - 1][k];
                dp1[i][j] += dp1[i - 1][j];
            }
            rep(j,1,10)++dp1[i][j];
            if (!flag)
            {
                ...
                {
                    flag = 1;
                    rep(j,0,tmp+1)++dp1[i][j];
                    rep(j,tmp+1,s[i]-'0') ++dp2[i][j];
                    tmp = s[i] - '0';
                }
            }
            elif (flag == 1)
            {
                ...
                {
                    sum += tmp;
                    rep(j,tmp,s[i]-'0')++dp2[i][j];
                    tmp = s[i] - '0';
                }
            }
            el sum += s[i] - '0';
            sum %= mod;
            rep(j,0,10)dp1[i][j] %= mod, dp2[i][j] %= mod;
        }
        if (flag == 2)++sum;
        ll ans=0;
        rep(i,0,len)ans = ((ans<<1)+(ans<<3) + s[i] - '0') % mod;
        ans = (ans - sum + mod) % mod;
        outl(ans);
    }
    rtn 0;
