...
static endln ln;
cst it inf=1044266558;
stt Point
{
	int x, y;
	Point ope - ( cst Point &b ) cst
	{
		Point c;
		c.x = x-b.x; c.y = y-b.y;
		rtn c;
	}
	db ope * ( cst Point &b ) cst
	{
		rtn x*b.y-y*b.x;
	}
};
...
il bl judge(Point x, Point y, Point z)
{
	rtn ((x.x<z.x && y.x<z.x) || (x.y<z.y && y.y<z.y) || (x.x>z.x && y.x>z.x) || (x.y>z.y && y.y>z.y));
}
int main(vt)
{
	it flag;
	...
	{
		msc(road,62);
		rep(i,1,n+1)fin>>h[i].x>>h[i].y;
		fin>>m;
		rep(i,1,m+1)fin>>s[i].x>>s[i].y;
		rep(i,1,m+1)
		{
			rep(j,1,m+1)
			{
				...
				rep(k,1,n+1)
				{
					if((s[i]-s[j])*(s[i]-h[k])<0 || (s[i]-s[j])*(s[i]-h[k])==0 && judge(s[i], s[j], h[k]))
					{
						flag = 0;
						break;
					}
