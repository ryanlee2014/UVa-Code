...
int solve()
{
    ...
    for(fa[q[0]=1]=0;L<=R;++L)
        for(int i=head[q[L]];~i;i=edges[i].next)
        {
            Edge &e=edges[i];
            if(!~fa[e.to])
                fa[q[++R]=e.to]=q[L];
    ...
    for(int i=R;i>=0;--i)
    {
        ...
        for(int i=head[u];~i;i=edges[i].next)
        {
            ...
            dp[u][0]+=max(dp[e.to][0],dp[e.to][1]);
            dp[u][1]+=max(dp[e.to][0],dp[e.to][1]);
            mx=max(mx,dp[e.to][0]+1-max(dp[e.to][0],dp[e.to][1]));
        }
        dp[u][1]+=mx;
    }
