...
stt Node
{
    ...
    Node()
    {}
    Node(it u, it v, it w) : u(u), v(v), w(w)
    {}
    bl ope<(cst Node &a) cst
    {
        if (u == 0)rtn true;
        rtn 1.0 * v / u > 1.0 * a.v / a.u;
    }
};
Node l[(it) (2 * 1e5 + 6)], r[(it) (2 * 1e5 + 6)];
ll sl, sr, ans;
ll llen, rlen;
int main(void)
{
    ...
    wl(T--)
    {
        ...
        {
            fin >> u >> v >> w;
            if (u > 0 || (u == 0 && v > 0))r[rlen++] = Node(u, v, w), sr += w;
            el
                l[llen++] = Node(u, v, w), sl += w;
        }
        ans = max(ans, sl * sr);
        sort(l, l + llen);
        sort(r, r + rlen);
        ...
        wl (lo < llen && ro < rlen)
        {
            db dl = 1.0 * l[lo].v / l[lo].u;
            db dr = 1.0 * r[ro].v / r[ro].u;
            if (dr > dl)
            {
                sr -= r[ro].w;
                sl += r[ro].w;
