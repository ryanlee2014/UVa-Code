...
bl vis[MAXN];
it sum[MAXN], len[MAXN];
it n, m;
int main()
{
    while(fin>>n>>m) {
        ms(vis);
        rep(i,0,n)fin>>nd[i].l>>nd[i].r;
        sort(nd, nd + n);
        rep(i,0,n)
        {
            int p = i;
            while(i + 1 < n && nd[i + 1].l <= nd[p].r) {
                vis[i + 1] = 1;
                nd[p].r = max(nd[p].r, nd[i + 1].r);
                i++;
            }
        }
        int ncnt = 0;
        rep(i,0,n)(!vis[i])&&(nnd[ncnt++]=nd[i],1);
        sum[0] = 0;
        len[0] = nnd[0].r - nnd[0].l + 1;
        rep(i,1,ncnt)
        {
            sum[i] = sum[i - 1] + nnd[i].l - nnd[i - 1].r - 1;
            len[i] = len[i - 1] + nnd[i].r - nnd[i].l + 1;
        }
        int ans = 0;
        rep(i,0,ncnt)
        {
            int pos = upper_bound(sum, sum + ncnt, sum[i] + m) - sum - 1;
