...
int dight[40],tmp[40];
ll dp[40][100][100][40];
ll dfs(int start,int pos,int s,bool limit)
{
    if(pos<0)
        return s;
    if(!limit&&dp[pos][s][start][ppp]!=-1)
        return dp[pos][s][start][ppp];
    int end;
    ll ret=0;
    if(limit)
        end=dight[pos];
...
int main()
{
    ...
    while (T--)
    {
        ...
        {
            ppp = i;
            temp = solve(y) - solve(x - 1);
            ans += ppp * temp + (y - x + 1 - temp);
        }
        fin<<"Case #"<<cnt++<<": "<<ans<<ln;
    }
    return 0;
